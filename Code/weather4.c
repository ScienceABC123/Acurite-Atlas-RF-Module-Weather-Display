...
/*******************************************************************************
*
* Function to draw wind gust
*
*******************************************************************************/

int DrawWindGust(int x, int y, int flip) {
	if (flip == 0) {
		DrawLine(x + 1, y - 3, x + 2, y - 3);
		DrawLine(x, y - 2, x, y - 2);
		DrawLine(x + 3, y - 2, x + 3, y - 2);
		DrawLine(x + 3, y - 1, x + 3, y - 1);
		DrawLine(x - 3, y, x + 2, y);
		flip = 1;
	} else {
		DrawLine(x - 3, y, x + 2, y);
		DrawLine(x + 3, y + 1, x + 3, y + 1);
...
/*******************************************************************************
*
* Function to draw lighting bolt
*
*******************************************************************************/

int DrawLightingBolt(int x, int y) {
	DrawLine(x + 1, y - 3, x + 1, y - 3);
	DrawLine(x, y - 2, x, y - 2);
	DrawLine(x - 1, y - 1, x - 1, y - 1);
...
*******************************************************************************/
		...
		if (i == 0) {
			...
			// Copy packet buffer to packet 0x03/0x10 buffer

			if (packet_buffer[0] == 0x03) {
				for (j = 0; j < 27; j++) {
					packet_0x03_buffer[j] = packet_buffer[j];
				}
			} else if (packet_buffer[0] == 0x10) {
				for (j = 0; j < 27; j++) {
...
*******************************************************************************/
			...
			switch(schedule_point) {
				...
				case 5:						// schedule point at 5 seconds (sample wind speed)
					...
					wind_peak = packet_0x10_buffer[9];
					wind_lull = packet_0x10_buffer[9];
				...
				case 15:					// schedule point at 15 seconds (sample wind speed)
					...
					if (wind_peak < packet_0x10_buffer[9]) {
						wind_peak = packet_0x10_buffer[9];
					}
					if (wind_lull > packet_0x10_buffer[9]) {
						wind_lull = packet_0x10_buffer[9];
					}
...
*******************************************************************************/
					...
					if ((temp_high_flag == 1) || (temp_low_flag == 1)) {
						...
						for (i = 0; i <= minute; i++) {
							...
							// Note: Graph temperature after dew point so it will be on top
...
*******************************************************************************/
					...
					if (fmaxf(wind_speed, (wind_gust_past[minute] + 1)) > wind_scale_high) {
						wind_speed_flag = 1;
					}
					...
					if (wind_speed_flag == 1) {
						...
						wind_scale_high = 5 * ceil(fmaxf(wind_speed, (wind_gust_past[minute] + 1)) / 5);
...
*******************************************************************************/
					...
					// Graph any previous and current lighting distances, on top of precipitation graph
					...
					if (prec_acc > 0) {
						SelectColors(BITTER_LEMON, BLACK);
						PrintStringLeft("Precip Rate:", x + 516, y - 298);

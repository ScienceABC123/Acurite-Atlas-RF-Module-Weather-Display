...
#define SUN_TRANSISTION 2						// Sun rise/set period in minutes (n - 1)
...
*******************************************************************************/
		...
		// Get current local date/time of system
...
*******************************************************************************/
					...
					// Get time zone													// Have to get the time zone here since it can change in the middle of the night

					strftime(str, 30, "%z", loc_time);
					time_zone = strtod(str, NULL) / 100;
					...
					if (strcmp(day_old, str) != 0) {
						...
						// New day: reset variables, erase previous past readings, and compute Julian date
						...
						a = 30.6001 * (b + 1);
						n = a;													// F
						julian_date = l + c + m + n - 1524.5;					// JD

						// Calculate Sunrise and Sunset
						// source: NOAA Solar Calculations spreadsheet

						aa = julian_date + (12.0 / 24.0) - (time_zone / 24.0);										// Julian day local time
						aa = (aa - 2451545.0) / 36525.0;															// Julian century
						...
						am = (720.0 - 4.0 * LONGITUDE - al + time_zone * 60.0) / 1440.0;							// Solar Noon
						...
						sunrise = a * 1440.0;																		// Sunrise in minutes
						a = am + an * 4.0 / 1440.0;																	// Sunset
						sunset = a * 1440.0;																		// Sunset in minutes
						...
						civil_twilight_am = a * 1440.0;																// Civil Twilight in the morning in minutes
						a = am + an * 4.0 / 1440.0;																	// Civil Twilight in the evening
						civil_twilight_pm = a * 1440.0;																// Civil Twilight in the evening in minutes
						...
						nautical_twilight_am = a * 1440.0;															// Nautical Twilight in the morning in minutes
						a = am + an * 4.0 / 1440.0;																	// Nautical Twilight in the evening
						nautical_twilight_pm = a * 1440.0;															// Nautical Twilight in the evening in minutes

						// Calculate the lunar day (i.e. number of days since the last full moon)
						// source: https://www.subsystems.us/uploads/9/8/9/4/98948044/moonphase.pdf
						// modified to use the April 8, 2024 20:00:00 gmt total solar eclipse

						a = ((julian_date - 2460409.3333333) + 0.5) / 29.53059;	// 2460409.3333333 = new moon on April 8, 2024 20:00:00 gmt
						i = a;
						b = roundf(29.53059 * (a - i));
						if (b >= 30) {
							b = 0;
						}
						lunar_day = b;
...
*******************************************************************************/
					...
					DrawRectFilled(x - 21, y - 243, x + 44, y - 198);

					// Display current solar description and draw sun or moon symbol
					// (source: arbitrary)
						...
						if (solar_adjusted >= 64) {
							if (uv >= 4) {
								if (solar_adjusted >= 1024) {
									...
									DisplayBMP(&Sunlight_BMP, x - 11, y - 243);
								}
							} else {
								sprintf(str, "Daylight");
								DisplayBMP(&Daylight_BMP, x - 11, y - 243);
							}
						} else {
							sprintf(str, "Dim Daylight");
							DisplayBMP(&Dim_Daylight_BMP, x - 11, y - 243);
						}
					} else {
						if ((minute >= nautical_twilight_am) && (minute < civil_twilight_am)) {
							sprintf(str, "Nautical Twilight");
						} else if ((minute >= civil_twilight_am) && (minute < (sunrise - SUN_TRANSISTION))) {
							sprintf(str, "Civil Twilight");
							DisplayBMP(&Twilight_BMP, x - 21, y - 243);
						} else if ((minute >= (sunrise - SUN_TRANSISTION)) && (minute <= sunrise)) {
							sprintf(str, "Sunrise");
							DisplayBMP(&Sunrise_Sunset_BMP, x - 21, y - 243);
						} else if ((minute >= sunset) && (minute <= (sunset + SUN_TRANSISTION))) {
							sprintf(str, "Sunset");
							DisplayBMP(&Sunrise_Sunset_BMP, x - 21, y - 243);
						} else if ((minute > (sunset + SUN_TRANSISTION)) && (minute <= civil_twilight_pm)) {
							sprintf(str, "Civil Twilight");
							DisplayBMP(&Twilight_BMP, x - 21, y - 243);
						} else if ((minute > civil_twilight_pm) && (minute <= nautical_twilight_pm)) {
							sprintf(str, "Nautical Twilight");
						} else {
							sprintf(str, "Night");
						}
						if ((minute < (sunrise - SUN_TRANSISTION)) || (minute > (sunset + SUN_TRANSISTION))) {
							switch(lunar_day) {
								case 0:
									DisplayBMP(&Moon_0_BMP, x + 543 , y - 243);
									break;
								case 1:
									DisplayBMP(&Moon_1_BMP, x + 543 , y - 243);
									break;
								case 2:
									DisplayBMP(&Moon_2_BMP, x + 543 , y - 243);
									break;
								case 3:
									DisplayBMP(&Moon_3_BMP, x + 543 , y - 243);
									break;
								case 4:
									DisplayBMP(&Moon_4_BMP, x + 543 , y - 243);
									break;
								case 5:
									DisplayBMP(&Moon_5_BMP, x + 543 , y - 243);
									break;
								case 6:
									DisplayBMP(&Moon_6_BMP, x + 543 , y - 243);
									break;
								case 7:
									DisplayBMP(&Moon_7_BMP, x + 543 , y - 243);
									break;
								case 8:
									DisplayBMP(&Moon_8_BMP, x + 543 , y - 243);
									break;
								case 9:
									DisplayBMP(&Moon_9_BMP, x + 543 , y - 243);
									break;
								case 10:
									DisplayBMP(&Moon_10_BMP, x + 543 , y - 243);
									break;
								case 11:
									DisplayBMP(&Moon_11_BMP, x + 543 , y - 243);
									break;
								case 12:
									DisplayBMP(&Moon_12_BMP, x + 543 , y - 243);
									break;
								case 13:
									DisplayBMP(&Moon_13_BMP, x + 543 , y - 243);
									break;
								case 14:
									DisplayBMP(&Moon_14_BMP, x + 543 , y - 243);
									break;
								case 15:
									DisplayBMP(&Moon_15_BMP, x + 543 , y - 243);
									break;
								case 16:
									DisplayBMP(&Moon_16_BMP, x + 543 , y - 243);
									break;
								case 17:
									DisplayBMP(&Moon_17_BMP, x + 543 , y - 243);
									break;
								case 18:
									DisplayBMP(&Moon_18_BMP, x + 543 , y - 243);
									break;
								case 19:
									DisplayBMP(&Moon_19_BMP, x + 543 , y - 243);
									break;
								case 20:
									DisplayBMP(&Moon_20_BMP, x + 543 , y - 243);
									break;
								case 21:
									DisplayBMP(&Moon_21_BMP, x + 543 , y - 243);
									break;
								case 22:
									DisplayBMP(&Moon_22_BMP, x + 543 , y - 243);
									break;
								case 23:
									DisplayBMP(&Moon_23_BMP, x + 543 , y - 243);
									break;
								case 24:
									DisplayBMP(&Moon_24_BMP, x + 543 , y - 243);
									break;
								case 25:
									DisplayBMP(&Moon_25_BMP, x + 543 , y - 243);
									break;
								case 26:
									DisplayBMP(&Moon_26_BMP, x + 543 , y - 243);
									break;
								case 27:
									DisplayBMP(&Moon_27_BMP, x + 543 , y - 243);
									break;
								case 28:
									DisplayBMP(&Moon_28_BMP, x + 543 , y - 243);
									break;
								case 29:
									DisplayBMP(&Moon_29_BMP, x + 543 , y - 243);
									break;
							}
						}
					}
					...
					// Display sunrise or sunset time, or lux value

					SelectFont(&Font5x9);
					SelectColors(BLACK, BLACK);
					DrawRectFilled(x - 40, y - 189, x + 92, y - 180);
					SelectColors(LT_GRAY, BLACK);

					// Display sunrise or sunset time

					if (LUX_FLAG == 0) {
						if ((minute <= sunrise) || (minute > (sunset + SUN_TRANSISTION))) {
							i = sunrise;
							sprintf(str, "Sunrise:");
						} else {
							i = sunset;
						...
						sprintf(str3, " %d:%02d", j, k);
						strcat(str, str3);
						strcat(str, str2);
						PrintStringCenter(str, x + 12, y - 189);
					}
					...
					if (LUX_FLAG == 1) {
						...
						if (a > 0) {
							...
							PrintStringLeft(str, x - 40, y - 189);
						}
					}

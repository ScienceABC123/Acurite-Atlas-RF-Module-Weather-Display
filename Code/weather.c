 ...
 *******************************************************************************/
		...
		while (i) {
			...
			if (j != -1) {
				...
				lux_raw = (packet_buffer[6] << 8) + packet_buffer[7];
		...
		while (i) {
			...
			if (j != -1) {
				...
				a = (lux_raw + ((packet_buffer[6] << 8) + packet_buffer[7])) / 2;
				lux_raw = 10 * a;
				...
				// Note: This is a little tricky since the two readings could be just on opposite sides of North
				...
				if ((a - b) < 180) {
					...
					wind_dir = fmodf(a + (((b + 360) - a) / 2), 360);
				}
...
*******************************************************************************/
		...
		if ((temp_high_flag == 1) || (temp_low_flag == 1)) {
			...
			// Graph any previous and current temperatures, dew points, wind chills and heat indexes
			...
			for (i = 0; i <= minute; i++) {
				...
				// Note: Graph dew points first in case of 100% humidity so temperatures will be graphed on top
				...
				if (dew_point_past[i] != -999) {
					...
					DrawCircleFilled(x + j, y - y_pos, 1);
				}
				...
				// Graph any previous and current wind chills or heat indexes
			...
			// Note: Graph dew point first in case of 100% humidity so temperature will be graphed on top
...
*******************************************************************************/
		...
		// Note: This is a little tricky since the two values could be just on opposite sides of North

		if (wind_dir_last == -1) {
			wind_dir_last = wind_dir;
		} else {
			a = fmaxf(wind_dir, wind_dir_last);
			b = fminf(wind_dir, wind_dir_last);
			wind_dir_last = wind_dir;
			if ((a - b) < 180) {
				wind_dir = (a + b) / 2;
			} else {
...
*******************************************************************************/
		...
		// Scaling to bring value back to 134,000 lux maximum

		b =  a / 15768.0;					// 15768 = 13,400 + 2,368
		c =  0.1502 * b;					// 0.1502 = 1 - (13,400 / 15,768)
		a = (1 - c) * a;

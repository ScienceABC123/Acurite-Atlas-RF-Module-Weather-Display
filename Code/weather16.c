...
#define HEAT_INDEX_TEMP_MINIMUM 0				// Heat index minus temperature minimum, below which heat index is ignored
...
#define LATITUDE 31.0416667						// Latitude of weather station, 31°02'30.0"N
#define LONGITUDE -97.3639444					// Longitude of weather station, 97°21'50.2"W

#define SAVE_HR_BMP_FLAG 1						// Save the display screen on the hour as a BMP

#define SAVE_EOD_BMP_FLAG 1						// Save the display screen at the end of the day as a BMP
...
int main(int argc, char* argv[]) {
	...
	int loc_minute_of_day_last;
	int loc_time_zone;
	int loc_time_zone_last = 13;
	int loc_time_zone_update_flag;
	int loc_day_of_week;
	...
	int low_battery_down_counter = 0;
	...
	int save_hr_bmp_flag = SAVE_HR_BMP_FLAG;
	int save_eod_bmp_flag = SAVE_EOD_BMP_FLAG;
...
*******************************************************************************/
	...
	while (j) {
		...
		// Check for start of next minute
...
*******************************************************************************/
		...
		SelectColors(BLACK, BLACK);
		DrawRectFilled(0, 0, 558, 9);
		if (debug_flag == 1) {
			SelectFont(&Font5x9);
			sprintf(str1, ":%02d  %02X  %02X  Timeout: %d  Checksum: %d  Empty Pkt: %d", second, packet_buffer[0], packet_buffer[25], timeout_error_count, checksum_error_count, empty_packet_error_count);
			SelectColors(LT_GRAY, BLACK);
			PrintStringLeft(str1, 0, 0);
		}

		// Check for schedule point

		if ((second == schedule_point) || (second == schedule_point + 1)) {
...
*******************************************************************************/
			...
			switch(schedule_point) {
				...
				case 55:					// schedule point at 55 seconds (sample all data and average over 1 minute)
					...
					uv = roundf((uv + (packet_0x10_buffer[5] & 0x0F)) / 2);
...
*******************************************************************************/
					...
					if (init_flag == 1) {
						...
						// Set local day of week variable to force new day update
...
*******************************************************************************/
					...
					// Compute wind direction averaged over last 10 minutes
...
*******************************************************************************/
				...
				case 0:						// schedule point at 0 seconds (update display)
					...
					file0 = fopen("options.h", "r");
					while (fscanf(file0, "%s %s %f", str1, str2, &a) != EOF) {
						if ((strcmp(str1, "#define") == 0) && (strcmp(str2, "LATITUDE") == 0)) {
							if ((a >= -90) && (a <= 90)) {
								if (latitude != a) {
									latitude = a;
									loc_time_zone_update_flag = 1;
								}
							}
						...
						} else if ((strcmp(str1, "#define") == 0) && (strcmp(str2, "SAVE_HR_BMP_FLAG") == 0)) {
							if (a != 0) {
								a = 1;
							}
							save_hr_bmp_flag = a;
						} else if ((strcmp(str1, "#define") == 0) && (strcmp(str2, "SAVE_EOD_BMP_FLAG") == 0)) {
							if (a != 0) {
								a = 1;
							}
							save_eod_bmp_flag = a;
						} else if ((strcmp(str1, "#define") == 0) && (strcmp(str2, "DEBUG_FLAG") == 0)) {
							if (a != 0) {
								a = 1;
							}
							debug_flag = a;
...
*******************************************************************************/
					...
					if (gmt_day_of_week != gmt_day_of_week_last) {
						...
						// Partial calculations for solar noon, sunrise/sunset, and civil and nautical twilight times
					...
					// Compute local minute of the day
					...
					// Check for local day update flag or local time zone update flag

					if ((loc_day_update_flag != 0) || (loc_time_zone_update_flag != 0)) {
						loc_time_zone_update_flag = 0;

						// Final calculations for solar noon, sunrise/sunset, and civil and nautical twilight times

						am = (720.0 - 4.0 * longitude - al + loc_time_zone * 60.0) / 1440.0;						// Solar Noon
						solar_noon = (am * 1440.0) + 0.5;
					...
					if (loc_day_update_flag != 0) {
						...
						SelectColors(WHITE, BLACK);
						PrintStringLeft(str1, 0, 950);

						// New day: reset variables, erase previous past readings, and set sun position flag
					...
					strcat(str1, str2);										// ex. "11:45am"
					SelectFont(&FontArial104);
					SelectColors(BLACK, BLACK);
					DrawRectFilled(1379, 950, 1919, 1079);
					SelectColors(WHITE, BLACK);
					PrintStringRight(str1, 1920, 950);

					// Compute x position for graphing

					if (loc_minute_of_day == 0) {
						loc_minute_of_day_last = 0;
					} else {
						loc_minute_of_day_last = loc_minute_of_day - 1;
					}
					a = loc_minute_of_day;
					b = x_size;
					c = b * (a / 1440);
...
*******************************************************************************/
					...
					// If battery low flag set, set low battery down counter for 1 day

					if (battery_low_flag != 0) {
						low_battery_down_counter = 1440;
					}

					// 	if low battery counter is not zero, display battery low symbol

					if (low_battery_down_counter != 0) {
						low_battery_down_counter = low_battery_down_counter - 1;
...
*******************************************************************************/
					...
					temp_past[loc_minute_of_day] = temp;

					// Save current dew point

					dew_point_past[loc_minute_of_day] = dew_point;

					// Save current wind chill/heat index

					wind_chill_heat_index_past[loc_minute_of_day] = fmaxf(wind_chill, heat_index);
					...
					sprintf(str1, "%.1f\xF8", temp);
					SelectColors(WHITE, BLACK);
					PrintStringCenter(str1, x + 290, y - 285);
					...
					if ((temp_high_flag == 1) || (temp_low_flag == 1)) {
						...
						SelectFont(&Font5x9);
						SelectColors(LT_GRAY, BLACK);
						sprintf(str1, "%d", ANNUAL_HIGH_TEMP);
						strcat(str1, "\xF8\x46");
						PrintStringLeft(str1, x + 24, y - (238 - i));
						sprintf(str1, "%d", ANNUAL_LOW_TEMP);
						strcat(str1, "\xF8\x46");
						PrintStringLeft(str1, x + 24, y - (192 - i));
						...
						if (dew_point_past[loc_minute_of_day_last] != -999) {
							y_pos_last = Y_Offset(dew_point_past[loc_minute_of_day_last], temp_scale_high, temp_scale_low, y_size);
						} else {
							y_pos_last = y_pos;
						}
						SelectColors(BRIGHT_APPLE_GREEN, BRIGHT_APPLE_GREEN);
						...
						if (wind_chill_heat_index_past[loc_minute_of_day] != -999) {
							y_pos = Y_Offset(wind_chill_heat_index_past[loc_minute_of_day], temp_scale_high, temp_scale_low, y_size);
							if (wind_chill_heat_index_past[loc_minute_of_day_last] != -999) {
								y_pos_last = Y_Offset(wind_chill_heat_index_past[loc_minute_of_day_last], temp_scale_high, temp_scale_low, y_size);
							} else {
								y_pos_last = y_pos;
							}
							if (wind_chill_heat_index_past[loc_minute_of_day] <= 50) {
								SelectColors(ICE_BLUE, ICE_BLUE);
							} else {
								SelectColors(BRIGHT_RED, BRIGHT_RED);
							}
						...
						if (temp_past[loc_minute_of_day_last] != -999) {
							y_pos_last = Y_Offset(temp_past[loc_minute_of_day_last], temp_scale_high, temp_scale_low, y_size);
						} else {
							y_pos_last = y_pos;
						}
						SelectColors(HOT_YELLOW, HOT_YELLOW);
					...
					if (TEMP_INSIDE_FLAG == 1) {
						...
						sprintf(str1, "Inside: %.1f\xF8", temp_inside);
						SelectColors(LT_GRAY, BLACK);
						PrintStringLeft(str1, x - 40, y - 245);
					}
					...
					if (prec_rate != 0) {
						...
						if (a > 122) {
							sprintf(str1, "Scorching");
						} else if (a > 112) {
							sprintf(str1, "Very Hot");
						} else if (a > 102) {
							sprintf(str1, "Hot");
						} else if (a > 92) {
							sprintf(str1, "Very Warm");
						} else if (a > 82) {
							sprintf(str1, "Warm");
						} else if (a > 72) {
							sprintf(str1, "Pleasant");
						} else if (a > 62) {
							sprintf(str1, "Cool");
						} else if (a > 52) {
							sprintf(str1, "Chilly");
							...
							sprintf(str1, "Very Cold");
						} else if (a > 22) {
							sprintf(str1, "Freezing");
						} else if (a > 12) {
							sprintf(str1, "Subfreezing");
						} else {
							sprintf(str1, "Arctic");
						}
						if (roundf(rh) > 60) {
					...
					PrintStringLeft(str1, x + 516, y - 298);
					...
					sprintf(str1, "%.1f\xF8", temp_high);
					PrintStringRight(str1, x + 691, y - 250);
					PrintStringLeft("Lo:", x + 707, y - 250);
					sprintf(str1, "%.1f\xF8", temp_low);
					PrintStringRight(str1, x + 889, y - 250);
					...
					} else if (wind_chill_heat_index_past[loc_minute_of_day_last] != -999) {
						if (wind_chill_heat_index_past[loc_minute_of_day_last] <= 50) {
							DrawRectFilled(x + 717, y - 202, x + 889, y - 163);			// Clear everything after "Wind Chill"
						} else if (wind_chill_heat_index_past[loc_minute_of_day_last] >= 70) {
							DrawRectFilled(x + 725, y - 202, x + 889, y - 163);			// Clear everything after "Heat Index"
						}
					}
					if (wind_chill_heat_index_past[loc_minute_of_day] != -999) {
						if (wind_chill_heat_index_past[loc_minute_of_day] <= 50) {
							sprintf(str1, "Wind Chill:");
							SelectColors(ICE_BLUE, BLACK);
						} else {
							sprintf(str1, "Heat Index:");
							SelectColors(BRIGHT_RED, BLACK);
						}
						PrintStringLeft(str1, x + 516, y - 202);
						sprintf(str1, "%.1f\xF8", wind_chill_heat_index_past[loc_minute_of_day]);
						SelectColors(WHITE, BLACK);
						PrintStringRight(str1, x + 889, y - 202);
					}
...
*******************************************************************************/
					...
					wind_speed_past[loc_minute_of_day] = wind_speed;

					// Check for wind gust

					if (wind_gust != 0) {
						wind_gust_past[loc_minute_of_day] = wind_gust;
						wind_gust = 0;
					}
					...
					sprintf(str1, "%.1f", wind_speed);
					SelectColors(WHITE, BLACK);
					PrintStringCenter(str1, x + 290, y - 285);

					// Check for wind speed scale change

					if (fmaxf(wind_speed, wind_gust_past[loc_minute_of_day]) > wind_scale_high) {
						wind_speed_flag = 1;
					}
					...
					if (wind_speed_flag == 1) {
						...
						wind_scale_high = 5 * ceil(fmaxf(wind_speed, wind_gust_past[loc_minute_of_day]) / 5);
						if (wind_scale_high < 10) {
							wind_scale_high = 10;
						} else if (wind_scale_high > 35) {
							wind_scale_high = 10 * ceil(wind_scale_high / 10);
						}
						...
						if (wind_scale_high == 10) {
							j = 2;
						} else if (wind_scale_high == 15) {
							j = 3;
						} else if (wind_scale_high < 40) {
							j = 5;
						} else {
							j = 10;
						}
...
//							sprintf(str1, "Dir (%d)", WIND_DIR_SPEED_THRESHOLD);
...
//							PrintStringRight(str1, x - 5, y - (y_pos + 4));
						...
						if (wind_speed_past[loc_minute_of_day_last] != -1) {
							y_pos_last = Y_Offset(wind_speed_past[loc_minute_of_day_last], wind_scale_high, 0, y_size);
						} else {
							y_pos_last = y_pos;
						}
						SelectColors(BRIGHT_SKY_BLUE, BRIGHT_SKY_BLUE);
					...
					SelectColors(BLACK, BLACK);
					DrawRectFilled(x + 11, y - 222, x + 74, y - 163);
					a = wind_speed;
					b = 17.26 * (sinf((WINDSOCK_ANGLE / 2) * (M_PI / 180.0)) / 60);	// Minimum speed for windsock to start to inflate
					if (a < b) {
						a = 0;
					}
					b = 17.26 * ((60 - sinf((WINDSOCK_ANGLE / 2) * (M_PI / 180.0))) / 60);	// Maximum speed before windsock "puffs" out
					if (a > b) {
						a = 17.26;
					}
					SelectColors(BLACK, BLACK);
					...
					if (wind_speed < 1) {
						...
						sprintf(str1, "Light Air");
					} else if (wind_speed < 8) {
						sprintf(str1, "Light Breeze");
					} else if (wind_speed < 13) {
						sprintf(str1, "Gentle Breeze");
					} else if (wind_speed < 19) {
						sprintf(str1, "Moderate Breeze");
					} else if (wind_speed < 25) {
						sprintf(str1, "Fresh Breeze");
					} else if (wind_speed < 32) {
						sprintf(str1, "Strong Breeze");
					} else if (wind_speed < 39) {
						sprintf(str1, "Near Gale");
					} else if (wind_speed < 47) {
						sprintf(str1, "Gale");
					} else if (wind_speed < 55) {
						sprintf(str1, "Strong Gale");
					} else if (wind_speed < 64) {
						sprintf(str1, "Whole Gale");
					} else if (wind_speed < 75) {
						sprintf(str1, "Storm Force");
					} else {
						sprintf(str1, "Hurricane Force");
					}
					SelectColors(WHITE, BLACK);
					PrintStringLeft(str1, x + 516, y - 298);
					...
					sprintf(str1, "%.1f", wind_speed_max);
					PrintStringRight(str1, x + 700, y - 250);
					PrintStringLeft("Min:", x + 716, y - 250);
					sprintf(str1, "%.1f", wind_speed_min);
					PrintStringRight(str1, x + 889, y - 250);
					...
					if (wind_gust_count != 0) {
						...
						SelectColors(WHITE, BLACK);
						sprintf(str1, "%d", wind_gust_count);
						PrintStringRight(str1, x + 889, y - 202);
					}
...
*******************************************************************************/
					...
					if (RH_INSIDE_FLAG == 1) {
						...
						SelectColors(BLACK, BLACK);
						DrawRectFilled(x - 40, y - 245, x + 92, y - 236);
						sprintf(str1, "Inside: %.0f%%", rh_inside);
						SelectColors(LT_GRAY, BLACK);
						PrintStringLeft(str1, x - 40, y - 245);
					}
					...
					sprintf(str1, "%.1f\xF8", dew_point);
					SelectColors(WHITE, BLACK);
					PrintStringRight(str1, x + 889, y - 298);
...
*******************************************************************************/
					...
					if (wind_dir_flag == 1) {
						...
						for (i = 0; i <= 1; i++) {
							...
							l = 198 + d;
							m = 198 - d;
							DrawLine(x + j, y - l, x + k, y - m);
						}
						SelectColors(BLACK, BLACK);
						DrawCircleFilled(x + 12, y - 198, 30);
						SelectColors(LT_GRAY, BLACK);
						for (i = 0; i <= 3; i++) {
							a = i;
							a = (45 * a) + 22.5;
						...
						SelectColors(BLACK, BLACK);
						DrawCircleFilled(x + 12, y - 198, 27);
						SelectColors(LT_GRAY, BLACK);
						for (i = 0; i <= 1; i++) {
							a = i;
							a = (90 * a) + 45;
						...
						if (WIND_DIR_SPEED_THRESHOLD != 0) {
							...
							sprintf(str1, "Wind Speed >= %d mph", WIND_DIR_SPEED_THRESHOLD);
							SelectColors(MED_GRAY, BLACK);
							PrintStringRight(str1, (x + x_size), y - (y_pos + 12));
						}
					}
					...
					SelectColors(BLACK, BLACK);
					DrawCircleFilled(x + 12, y - 198, 25);
					SelectFont(&Font5x9);
					SelectColors(LT_GRAY, BLACK);
					PrintStringCenter("N", x + 13, y - 227);
					PrintStringCenter("E", x + 39, y - 202);
					PrintStringCenter("S", x + 13, y - 177);
					...
					j = 12 + c;
					k = 198 + d;
					DrawThickLine(x + 12, y - 198, x + j, y - k);
...
*******************************************************************************/
					...
					prec_acc_past[loc_minute_of_day] = prec_acc;
					...
					if (loc_minute_of_day >= 3) {
						prec_rate_past[loc_minute_of_day - 3] = prec_rate;
					}
					...
					if (prec_flag == 1) {
						...
						if (prec_acc > 0) {
							...
							PrintStringRight(str1, x - 5, y - (y_size + 30));
						}
						if (ltng_acc > 0) {
							sprintf(str1, "miles");
							SelectColors(BRIGHT_RED, BLACK);
							PrintStringLeft(str1, x + (x_size + 7), y - (y_size + 18));
						}
						for (a = 0; a <= 5; a++) {
							y_pos = Y_Offset(a, 5, 0, y_size);
							sprintf(str1, "%.1f", prec_scale_high * (a / 5));
							SelectColors(LT_GRAY, BLACK);
							PrintStringRight(str1, x - 5, y - (y_pos + 4));
							if (ltng_acc > 0) {
								sprintf(str1, "%.0f", (a * 5));
								PrintStringLeft(str1, x + (x_size + 7), y - (y_pos + 4));
							}
							SelectColors(MED_GRAY, MED_GRAY);
							DrawLine(x, y - y_pos, x + (x_size - 1), y - y_pos);
						}
					...
					if (ltng_dist_flag == 1) {
						ltng_dist_flag = 0;
						ltng_dist_past[loc_minute_of_day] = ltng_dist;
					}
					for (i = 0; i <= loc_minute_of_day; i++) {
					...
					if (BARO_FLAG == 1) {
						...
						sprintf(str1, "%.2f inHg", barometric_pressure);
						PrintStringRight(str1, x + 889, y - 298);
					}
					...
					if (ltng_acc > 0) {
						...
						if (a < 1000) {
							...
							sprintf(str1, "%.0f,%03.0f", (a / 1000), fmodf(a, 1000));
						}
						SelectColors(WHITE, BLACK);
						PrintStringRight(str1, x + 889, y - 250);
					}
...
*******************************************************************************/
					...
					solar_adjusted_past[loc_minute_of_day] = solar_adjusted;
					...
					if (solar_flag == 1) {
						...
						y_pos = Y_Offset(solar_adjusted_past[loc_minute_of_day], solar_scale_high, 0, y_size);
						if (solar_adjusted_past[loc_minute_of_day_last] != -1) {
							y_pos_last = Y_Offset(solar_adjusted_past[loc_minute_of_day_last], solar_scale_high, 0, y_size);
						} else {
							y_pos_last = y_pos;
						}
						SelectColors(SUN_YELLOW, SUN_YELLOW);
					...
					// Display latitude, longitude, and sun times

					if ((LUX_FLAG == 0) && (sun_times_flag == 1)) {
						sun_times_flag = 0;

						SelectFont(&Font5x9);
						SelectColors(BLACK, BLACK);
						DrawRectFilled(x - 80, y - 244, x + 115, y - 179);
						SelectColors(LT_GRAY, BLACK);
						...
						PrintStringLeft(str1, x - 80, y - 244);
						...
						PrintStringRight(str1, x + 115, y - 244);

						// Solar noon

						PrintStringRight("Solar Noon:", x + 25, y - 230);
						i = solar_noon;
						j = i / 60;
						k = i - (j * 60);
						if (j <= 11) {
						...
						strcat(str1, str2);
						PrintStringCenter(str1, x + 72, y - 230);

						// Sunrise/Sunset

						PrintStringRight("Sunrise/Sunset:", x + 25, y - 216);
						i = sunrise;
						j = i / 60;
						k = i - (j * 60);
						if (j <= 11) {
						...
						sprintf(str1, "%2d:%02d", j, k);
						strcat(str1, str2);
						PrintStringRight(str1, x + 70, y - 216);
						i = sunset;
						j = i / 60;
						k = i - (j * 60);
						if (j <= 11) {
						...
						sprintf(str1, "%2d:%02d", j, k);
						strcat(str1, str2);
						PrintStringRight(str1, x + 115, y - 216);

						// Civil twilight

						PrintStringRight("Civil Twilight:", x + 25, y - 202);
						i = civil_twilight_am;
						j = i / 60;
						k = i - (j * 60);
						if (j <= 11) {
						...
						sprintf(str1, "%2d:%02d", j, k);
						strcat(str1, str2);
						PrintStringRight(str1, x + 70, y - 202);
						i = civil_twilight_pm;
						j = i / 60;
						k = i - (j * 60);
						if (j <= 11) {
						...
						sprintf(str1, "%2d:%02d", j, k);
						strcat(str1, str2);
						PrintStringRight(str1, x + 115, y - 202);

						// Nautical twilight

						PrintStringRight("Nautical Twilight:", x + 25, y - 188);
						...
						sprintf(str1, "%2d:%02d", j, k);
						strcat(str1, str2);
						PrintStringRight(str1, x + 70, y - 188);
						...
						sprintf(str1, "%2d:%02d", j, k);
						strcat(str1, str2);
						PrintStringRight(str1, x + 115, y - 188);
					}
					...
					if (LUX_FLAG == 1) {
						...
						SelectFont(&Font5x9);
						SelectColors(BLACK, BLACK);
						DrawRectFilled(x - 42, y - 240, x + 117, y - 231);
						SelectColors(LT_GRAY, BLACK);
						...
						if (a > 0) {
							...
							strcat(str1, " lux (adjusted)");
							PrintStringLeft(str1, x - 42, y - 240);
						}
					}
					...
					SelectFont(&FontArial32);
					SelectColors(BLACK, BLACK);
					DrawRectFilled(x + 516, y - 298, x + 889, y - 163);

					// Display current solar description and moon symbol
					// (source: solar description arbitrary, moon images from https://print.intervalleyproject.org/about-us/)

					if ((loc_minute_of_day > sunrise) && (loc_minute_of_day != solar_noon) && (loc_minute_of_day < sunset)) {
						if (solar_adjusted >= 1000) {
							sprintf(str1, "Bright Sunlight");
						} else if (solar_adjusted >= 800) {
							sprintf(str1, "Sunlight");
						} else if (solar_adjusted >= 64) {
							sprintf(str1, "Daylight");
						} else {
							sprintf(str1, "Dim Daylight");
						}
					} else {
						if ((loc_minute_of_day >= nautical_twilight_am) && (loc_minute_of_day < civil_twilight_am)) {
							sprintf(str1, "Nautical Twilight");
						} else if ((loc_minute_of_day >= civil_twilight_am) && (loc_minute_of_day < sunrise)) {
							sprintf(str1, "Civil Twilight");
						} else if (loc_minute_of_day == sunrise) {
							sprintf(str1, "Sunrise");
						} else if (loc_minute_of_day == solar_noon) {
							sprintf(str1, "Solar Noon");
						} else if (loc_minute_of_day == sunset) {
							sprintf(str1, "Sunset");
						} else if ((loc_minute_of_day > sunset) && (loc_minute_of_day <= civil_twilight_pm)) {
							sprintf(str1, "Civil Twilight");
						} else if ((loc_minute_of_day > civil_twilight_pm) && (loc_minute_of_day <= nautical_twilight_pm)) {
							sprintf(str1, "Nautical Twilight");
						} else {
							sprintf(str1, "Night");
						}
						if ((loc_minute_of_day < sunrise) || (loc_minute_of_day > sunset)) {
							...
							DisplayBMP(bmp_adr, x + 534 , y - 243);			// Display moon BMP
						}
					}
					SelectColors(WHITE, BLACK);
					PrintStringLeft(str1, x + 516, y - 298);
					...
					if ((loc_minute_of_day >= sunrise) && (loc_minute_of_day <= sunset) && (uv > 0)) {
						...
						if (uv >= 11) {
							sprintf(str1, "Extreme");
						} else if (uv >= 8) {
							sprintf(str1, "Very High");
						} else if (uv >= 6) {
							sprintf(str1, "High");
						} else if (uv >= 3) {
							sprintf(str1, "Moderate");
						} else {
							sprintf(str1, "Low");
						}
						SelectColors(WHITE, BLACK);
					...
					if (((save_hr_bmp_flag != 0) && (loc_minute == 0)) || ((save_eod_bmp_flag != 0) && (loc_hour_24 == 23) && (loc_minute == 59))) {

						// Create filename

						strftime(filename, 60, "/home/pi/Weather/Print_Screens/Weather_%Y%m%d-%H%ML.bmp", current_time);

						// Open the file for writing

						file0 = fopen(filename, "wb");
